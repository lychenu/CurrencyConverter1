...
public class TransactionProcessor {
	...
	public void executeTransactions() throws IOException {
		if(userList.containsKey(name) == false) {
			// skip transaction
		;	
		}
		else if( !userList.containsKey(toCurrency)) {
			// execute deposit
	...
	public void executeDeposit() {
		if(userList.get(name).getWallet().containsKey(fromCurrency)) {
			double amountInWallet = userList.get(name).getWallet().get(fromCurrency);
			amountInWallet = amountInWallet - amount;
		}
	}
